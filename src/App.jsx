...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubGroups = onSnapshot(groupsRef, (snapshot) => {
      if (snapshot.empty) {
        // 初期データの投入（パスワードは一律 '1234'）
        const batch = writeBatch(db);
        INITIAL_GROUPS.forEach(g => {
          const docRef = doc(groupsRef);
          batch.set(docRef, { ...g, password: '1234', createdAt: new Date().toISOString() });
        });
  ...
  const handlePortalLogin = (e) => {
    ...
    const group = groups.find(g => 
      ...
      (g.password === portalPassInput || (!g.password && portalPassInput === '1234')) // 初期設定フォールバック
    );
  ...
  const handleAdminLogout = async () => {
    try {
      ...
      setActiveTab('calendar'); setIsPortalAuthorized(false); setLoggedInGroup(null);
      showToast('ログアウトしました');
    } catch (error) {}
  };
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-md text-center space-y-8 animate-in zoom-in duration-500 z-10">
          ...
          {isLocked ? (
            ...
            <form onSubmit={handlePortalLogin} className="space-y-6">
              ...
              <p className="text-[10px] text-gray-400 font-bold px-4 text-left">※初期パスワードは「1234」です。</p>
                    ...
                    >
                      {isOwnReservation && (
                        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                          <button onClick={(e) => { e.stopPropagation(); onEditClick(res); }} className="bg-orange-100 text-orange-700 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-orange-200 flex items-center shadow-sm transition-colors border border-orange-200">
...
function AdminDashboard({ reservations, closedDays, groups, reports, currentAnnouncement, penaltySettings, onStatusUpdate }) {
  ...
  const [newGroupPassword, setNewGroupPassword] = useState('1234'); // ★ パスワード追加
          ...
          <form onSubmit={handleAddGroup} className="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-100">
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
              ...
              <div className="sm:col-span-3">
                ...
                <input type="text" required placeholder="1234" value={newGroupPassword} onChange={(e)=>setNewGroupPassword(e.target.value)} className="w-full border p-2.5 rounded-xl text-sm font-bold outline-none focus:border-indigo-500 tracking-widest font-mono" />
              </div>
            </div>
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex flex-col w-full">
                        ...
                        <div className="mt-1.5 flex justify-between items-center bg-gray-50 border rounded p-1">
                           <span className="text-[9px] font-bold text-gray-500">PASS: <span className="font-mono text-gray-800">{g.password || '未設定(1234)'}</span></span>
